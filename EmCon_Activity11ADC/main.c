...
float ki = 2; // integral control gain
...
int main(void)
{
    ...
    while(1){
        ...
        if(timer_flag){
            ...
            while (ADC14_isBusy()){}
            uint16_t pot_val = ADC14_getResult(ADC_MEM0);
            //N_ADC = (V_R+ - V_R-) * ((V_in+ - V_R-) / (2^N_bits)) /// N_bits = resolution = 14, V_in+ = 3.3, V_R- = 0
            desired = ((pot_val - 0) * (3.3 - 0)) / (0x01<<14);

            uint16_t rc_val = ADC14_getResult(ADC_MEM1);
            actual = ((rc_val - 0) * (3.3 - 0)) / (0x01<<14);
...
void GPIOInit(){
    ...
    // Add ADC pins
    GPIO_setAsPeripheralModuleFunctionOutputPin(GPIO_PORT_P6, GPIO_PIN1, GPIO_TERTIARY_MODULE_FUNCTION);
    GPIO_setAsPeripheralModuleFunctionOutputPin(GPIO_PORT_P6, GPIO_PIN0, GPIO_TERTIARY_MODULE_FUNCTION);
}


void ADCInit(){
    ADC14_enableModule();
    //Need 6MHz for ADC ; SMCLK=24MHz, PreDiv=4, Div=1 -> (24/4)/1 = 6
    ADC14_initModule(ADC_CLOCKSOURCE_SMCLK, ADC_PREDIVIDER_4, ADC_DIVIDER_1, ADC_NOROUTE);
    ADC14_setResolution(ADC_14BIT); //x=8,10,12,14
